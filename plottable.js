    ...
    (function (Components) {
        var Legend = (function (_super) {
            ...
            Legend.prototype.renderImmediately = function () {
                ...
                entries.select("path").attr("d", function (d, i, j) { return _this.symbol()(d, j)(layout.textHeight * 0.6); }).attr("transform", "translate(" + (layout.textHeight / 2) + "," + layout.textHeight / 2 + ")").attr("fill", function (value) { return _this._colorScale.scale(value); }).attr("opacity", function (d, i) { return _this.symbolOpacity()(d, i); }).classed(Legend.LEGEND_SYMBOL_CLASS, true);
            ...
            Bar.prototype._drawLabel = function (data, dataset) {
                ...
                var labelTooWide = data.map(function (d, i) {
                    ...
                    if (measurement.height <= h && measurement.width <= w) {
                        ...
                        if (labelPosition.x < 0 || labelPosition.x + measurement.width > _this.width() || labelPosition.y < 0 || labelPosition.y + measurement.height > _this.height()) {
                            showLabel = false;
                        }
                        var g = labelArea.append("g").attr("transform", "translate(" + x + "," + y + ")");
                        var className = dark ? "dark-label" : "light-label";
        ...
        var DragBoxLayer = (function (_super) {
            ...
            DragBoxLayer.prototype._setUpCallbacks = function () {
                ...
                this._dragInteraction.onDragStart(function (startPoint) {
                    ...
                    var isInsideBox = bounds.topLeft.x <= startPoint.x && startPoint.x <= bounds.bottomRight.x && bounds.topLeft.y <= startPoint.y && startPoint.y <= bounds.bottomRight.y;
